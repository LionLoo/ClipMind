...
def check_near_duplicates(text: str, store: IndexStore, similarity_threshold: float = 0.95) -> Optional[tuple]:
    """
    Check if very similar item exists with FAISS

    Args:
        text: text to check
        store: FAISS IndexStore
    ...
    distances, positions, item_ids = store.search(vector, top_k = 1)
    if len(item_ids) == 0 or item_ids[0] == -1:
        return None
    # Converts L2 distance to cos similarity
    # For normalized vectors: low distance = high similarity
    #if distance is small, likely near dup
    #threshold: distance < 0.3 ~ >95% similar
