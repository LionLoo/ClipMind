...
#   added support for text search, image search, filtering by source
...
def semantic_search(
    ...
    if mode in ["all", "text"]:
        if store.text_index.ntotal > 0:
            query_vector = encode_text_to_vector(query_text)
            distances, positions, item_ids = store.search_text(query_vector, top_k)
            ...
            with get_session() as session:
                for item_id, distance in zip(item_ids, distances[0]):
                    ...
                    if item:
                        ...
                        if source_filter is None or item.source == source_filter:
    ...
    if mode in ["all", "images"]:
        if store.image_index.ntotal > 0:
            ...
            distances, positions, item_ids = store.search_image(query_vector, top_k)
...
def search_images_only(query_text: str, top_k: int = None) -> List[Tuple[Item, float]]:
    """
...
def search_text_only(query_text: str, top_k: int = None) -> List[Tuple[Item, float]]:
    """
    Search only text content (clipboard items + screenshot OCR)
    """
    return semantic_search(query_text, top_k=top_k, mode="text")


def search_clipboard_only(query_text: str, top_k: int = None) -> List[Tuple[Item, float]]:
    """
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m app.search.semantic_search <query> [top_k] [mode]")
    ...
    if mode == "images":
        results = search_images_only(query, top_k)
    elif mode == "clipboard":
        results = search_clipboard_only(query, top_k)
    elif mode == "text":
        results = search_text_only(query, top_k)
    else:
        results = semantic_search(query, top_k, mode="all")
